...
pub fn copy_folder_content(folder_path: &str) -> bool {
    let folder_path_is_folder = fs::metadata(folder_path).unwrap().is_dir();
    return if folder_path_is_folder {
        let folder_content = fs::read_dir(folder_path).unwrap();
        let folder_name = folder_path.split("\\").last().unwrap();
        let create_folder_action = fs::create_dir(format!("./{}", folder_name));
        match create_folder_action {
            Ok(_) => {
                ...
                for file in folder_content {
                    ...
                    let copy_action = fs::copy(file_path.to_str().unwrap(), format!("./{}/{}", folder_name, file_name.clone()));
                    match copy_action {
                        Ok(_) => {
                 ...
                 for file in folder_content {
                     ...
                     match file_content {
                         Ok(mut content) => {
                             ...
                             fs::write(format!("./{}/{}", folder_name, name), content).unwrap();
                         ...
                         Err(_) => {
                             if copy_folder_content_with_vars(format!("{}/{}", folder_path, file_name).as_str(), vars_setters, vars_values) {
                                 ...
                                 let copy_action = fs::copy(file_path.clone(), format!("./{}/{}", folder_name, file_name));
                                 match copy_action {
                                     Ok(_) => {
                                         logger::info(format!("Copiado {}", file_name).as_str());
                                         return true
